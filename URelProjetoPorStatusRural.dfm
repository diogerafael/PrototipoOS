inherited FrmRelProjetoStatusRural: TFrmRelProjetoStatusRural
  Left = 378
  Top = 30
  Caption = '+'
  ClientHeight = 606
  ClientWidth = 650
  OnShow = FormShow
  ExplicitTop = -62
  ExplicitWidth = 656
  ExplicitHeight = 635
  PixelsPerInch = 96
  TextHeight = 13
  inherited g1: TGauge
    Left = 18
    Top = 532
    Width = 624
    ExplicitLeft = 18
    ExplicitTop = 532
    ExplicitWidth = 624
  end
  inherited shp1: TShape
    Width = 650
    ExplicitLeft = 273
    ExplicitTop = 8
    ExplicitWidth = 650
  end
  inherited lbl1: TLabel
    Left = 273
    Width = 115
    Caption = 'Relat'#243'rio Rural'
    ExplicitLeft = 273
    ExplicitWidth = 115
  end
  inherited pnl1: TPanel
    Top = 552
    Width = 650
    TabOrder = 9
    ExplicitTop = 552
    ExplicitWidth = 650
    inherited btnsair: TPngSpeedButton
      Left = 306
      ExplicitLeft = 306
    end
    inherited btnrelatorio: TPngSpeedButton
      Left = 199
      OnClick = btnrelatorioClick
      ExplicitLeft = 199
    end
    object btnExportar: TBitBtn
      Left = 413
      Top = 15
      Width = 109
      Height = 26
      Cursor = crHandPoint
      Caption = '&Exportar Excel'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        0003737FFFFFFFFF7F7330099999999900333777777777777733}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = btnExportarClick
    end
    object btnExportarDevolvidas: TBitBtn
      Left = 533
      Top = 15
      Width = 109
      Height = 26
      Cursor = crHandPoint
      Caption = 'Exp. &Devolvidas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        0003737FFFFFFFFF7F7330099999999900333777777777777733}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
      OnClick = btnExportarDevolvidasClick
    end
  end
  object grp1: TGroupBox [5]
    Left = 15
    Top = 75
    Width = 338
    Height = 62
    Caption = 'Per'#237'odo Chegada'
    TabOrder = 0
    object lbl3: TLabel
      Left = 4
      Top = 30
      Width = 58
      Height = 14
      Caption = 'Data Inicial'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object lbl2: TLabel
      Left = 165
      Top = 30
      Width = 52
      Height = 14
      Caption = 'Data Final'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object edtini: TcxDateEdit
      Left = 68
      Top = 27
      ParentFont = False
      Properties.DateOnError = deNull
      Style.BorderStyle = ebsUltraFlat
      Style.Color = clWindow
      Style.Font.Charset = ANSI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -12
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.ButtonStyle = btsDefault
      Style.ButtonTransparency = ebtHideUnselected
      Style.IsFontAssigned = True
      StyleFocused.Color = clMoneyGreen
      TabOrder = 0
      Width = 90
    end
    object edtfim: TcxDateEdit
      Left = 223
      Top = 27
      ParentFont = False
      Properties.DateOnError = deNull
      Style.BorderStyle = ebsUltraFlat
      Style.Color = clWindow
      Style.Font.Charset = ANSI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -12
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.ButtonStyle = btsDefault
      Style.ButtonTransparency = ebtHideUnselected
      Style.IsFontAssigned = True
      StyleFocused.Color = clMoneyGreen
      TabOrder = 1
      Width = 90
    end
  end
  object grp2: TGroupBox [6]
    Left = 18
    Top = 361
    Width = 624
    Height = 74
    TabOrder = 5
    object chkcadastrada: TCheckBox
      Left = 21
      Top = 3
      Width = 97
      Height = 17
      Caption = 'Cadastrada'
      TabOrder = 0
    end
    object chkLevantamento: TCheckBox
      Left = 21
      Top = 27
      Width = 97
      Height = 17
      Caption = 'Levantamento'
      TabOrder = 6
    end
    object chkImprocedente: TCheckBox
      Left = 21
      Top = 50
      Width = 97
      Height = 17
      Caption = 'Improcedente'
      TabOrder = 8
    end
    object chksoltopografia: TCheckBox
      Left = 146
      Top = 50
      Width = 97
      Height = 17
      Caption = 'Sol. Topografia'
      TabOrder = 9
    end
    object chkTopografia: TCheckBox
      Left = 146
      Top = 3
      Width = 97
      Height = 17
      Caption = 'Topografia'
      TabOrder = 1
    end
    object chkprojeto: TCheckBox
      Left = 146
      Top = 27
      Width = 117
      Height = 17
      Caption = 'Elabora'#231#227'o Projeto'
      TabOrder = 7
    end
    object chkConcluido: TCheckBox
      Left = 312
      Top = 3
      Width = 76
      Height = 17
      Caption = 'Conclu'#237'do'
      TabOrder = 2
    end
    object chkTodos: TCheckBox
      Left = 445
      Top = 23
      Width = 97
      Height = 17
      Caption = 'Todos'
      TabOrder = 5
      OnClick = chkTodosClick
    end
    object chkDevolvida: TCheckBox
      Left = 312
      Top = 23
      Width = 76
      Height = 17
      Caption = 'Devolvida'
      TabOrder = 4
    end
    object chkConImprocedente: TCheckBox
      Left = 312
      Top = 50
      Width = 138
      Height = 17
      Caption = 'Conclu'#237'do Improcedente'
      TabOrder = 10
    end
    object chkTopLiberada: TCheckBox
      Left = 445
      Top = 3
      Width = 138
      Height = 17
      Caption = 'Topografia Liberada'
      TabOrder = 3
    end
  end
  object grpFuncionario: TGroupBox [7]
    Left = 19
    Top = 295
    Width = 624
    Height = 58
    Caption = 'Funcion'#225'rio'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    object edtcodtecnico: TEdit
      Left = 16
      Top = 19
      Width = 86
      Height = 22
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      OnKeyPress = edtcodtecnicoKeyPress
    end
    object edtdesctecnico: TEdit
      Left = 136
      Top = 19
      Width = 457
      Height = 22
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
    end
  end
  object rgGroup: TRadioGroup [8]
    Left = 19
    Top = 239
    Width = 624
    Height = 44
    Caption = 'Perfil Funcionario'
    Columns = 4
    ItemIndex = 3
    Items.Strings = (
      'Levantador'
      'Topografo'
      'Projetista'
      'Nenhum')
    TabOrder = 3
  end
  object rgTipo: TRadioGroup [9]
    Left = 371
    Top = 75
    Width = 272
    Height = 62
    Caption = 'Tipo'
    Columns = 3
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ItemIndex = 0
    Items.Strings = (
      'Ativo'
      'Passivo'
      'Ambos')
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
  end
  object rgTpPeriodo: TRadioGroup [10]
    Left = 19
    Top = 144
    Width = 624
    Height = 87
    Caption = 'Per'#237'odo'
    Columns = 4
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ItemIndex = 0
    Items.Strings = (
      'Rec. Energisa'
      'Lib. Levantamento'
      'Ret. Levantamento'
      'Lib. Topografia'
      'Ret. Topografia'
      'Lib. Projetar'
      'Ret. Projetar'
      'Devolvida'
      'Conclus'#227'o'
      'Sol. Topografia'
      'Dt. Lib. Top')
    ParentFont = False
    TabOrder = 2
  end
  object grpOrdenacao: TGroupBox [11]
    Left = 18
    Top = 489
    Width = 287
    Height = 38
    Caption = 'Ordena'#231#227'o'
    TabOrder = 7
    object chkPeriodo: TCheckBox
      Left = 35
      Top = 13
      Width = 97
      Height = 17
      Caption = 'Per'#237'odo'
      TabOrder = 0
    end
    object chkStatus: TCheckBox
      Left = 155
      Top = 13
      Width = 97
      Height = 17
      Caption = 'Status'
      TabOrder = 1
    end
  end
  object grpmunicipio: TGroupBox [12]
    Left = 19
    Top = 441
    Width = 623
    Height = 41
    Caption = 'Municipio'
    TabOrder = 6
    object edtcodmunicipio: TEdit
      Left = 20
      Top = 14
      Width = 82
      Height = 19
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      NumbersOnly = True
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      OnKeyPress = edtcodmunicipioKeyPress
    end
    object edtdescmunicipio: TEdit
      Left = 148
      Top = 14
      Width = 445
      Height = 19
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
    end
  end
  object grp3: TGroupBox [13]
    Left = 336
    Top = 489
    Width = 306
    Height = 38
    Caption = 'Universaliza'#231#227'o'
    TabOrder = 8
    object edtUniversalizacao: TEdit
      Left = 93
      Top = 10
      Width = 121
      Height = 21
      NumbersOnly = True
      TabOrder = 0
    end
  end
  inherited rdprnt1: TRDprint
    Left = 492
    Top = 24
  end
  inherited pm1: TPopupMenu
    Left = 424
    Top = 7
    object E1: TMenuItem
      Caption = 'Exporta'#231#227'o Devolvidas'
      OnClick = E1Click
    end
  end
  object SaveDialog1: TSaveDialog
    Filter = 'Excel|xls'
    Left = 560
    Top = 480
  end
  object ThreadProcessando: TJvThread
    Exclusive = True
    MaxCount = 0
    RunOnCreate = True
    FreeOnTerminate = True
    OnExecute = ThreadProcessandoExecute
    Left = 593
    Top = 487
  end
end
