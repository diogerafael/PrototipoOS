...
begin
  ...
  if Obrigatorio(Self,edtdtenviorevisao,'Inserir Inicio da Revisão')then exit;
  ...
  Application.MessageBox('Revisão Editada com Sucesso !', 'Informação',
...
begin
  ...
  if Obrigatorio(Self,edtdtenviorevisao,'Inserir Inicio da Revisão !')then exit;
  if Obrigatorio(Self,edtdescprojetistarevisao,'Inserir Responsável !') then Exit;
  ...
  begin
    ...
    begin
      ShowMessage('Nescessário Salvar o Projeto Para Inserir Uma Revisão');
    end
    else
    begin
      if(Self.Tag=0)then
      begin
             ...
             Application.MessageBox('Revisão Inserida com Sucesso !', 'Informação',
               MB_OK + MB_ICONINFORMATION);
      end
      else if(Self.Tag=1)then
      begin
