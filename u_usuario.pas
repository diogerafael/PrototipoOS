...
begin
  ...
  begin
    ...
    if Obrigatorio(Self, edtusuario, 'Informe o Usuário !') then Exit;
    ...
    if frmusuario.Tag = 1 then //Liberar Acesso no Parâmetro do Sistema
    begin
      _liberaacesso := false;
      ...
      Application.MessageBox('Código Inválido!', 'Atenção!', MB_OK +MB_ICONWARNING);
    end;
    {
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox('Atenção! Usuário Sem Permissão!',PChar(Application.Title), MB_OK + MB_ICONINFORMATION);
          _autorizouvendasuperior := False;
          //frmusuario.close;
          Abort;
...
//      frmusuario.close;
    ...
    {
    if frmusuario.Tag = 3 then //Liberar Desconto Item da Locação
    begin
      _liberaacesso := False;
      if permissao = 0 then
      begin
    ...
    if frmusuario.Tag = 4 then //Autorizar o Cancelamento da Locação
    begin
      _liberaacesso := False;
      if permissao = 0 then
      begin
    ...
    if frmusuario.Tag = 6 then //Liberar Formação de Preço de Venda Rápida
    begin
      ...
      if ConsultaSql(datamodule1.qry,strSql) <> 0 then
      begin
        Application.MessageBox('OPÇÃO NÃO DISPONÍVEL PARA ESTE USUÁRIO','Atenção!', MB_OK + MB_ICONWARNING);
        _liberaacesso :=  False;
        edtsenha.SetFocus;
        exit;
      end
    ...
    if frmusuario.Tag = 8 then //ALTERAR PREÇO DOS PRODUTOS POR DEPARTAMENTALIZAÇÃO
    begin
    ...
    if frmusuario.Tag = 9 then //VINCULAÇÃO AUTOMATICA DE TODOS OS PRODUTOS AO LOTE
    begin
      _grupo                :=  '00';
      sqlstr := ''+
      'select perfil from uccaduser where login = '+trim(edtusuario.text);
    ...
    if frmusuario.Tag = 10 then //DEPARTAMENTALIZAÇÃO/PRODUTOS COM COMISSAO DIFERENCIADA
    begin
      _grupo                :=  '00';
      sqlstr := ''+
      'select perfil from uccaduser where login = '+trim(edtusuario.text);
    ...
    //libera acesso para alterar todos os departamento mercadológico....
    ...
    if frmusuario.Tag = 15 then //EXCLUIR USUÁRIOS INATIVOS
    begin
    ...
    begin
      ...
      if ConsultaSql(datamodule1.qry,sqlstr) <> 0 then
      begin
        Application.MessageBox('Atenção! Usuário Não Cadastrado!',PChar(Application.Title), MB_OK + MB_ICONWARNING);
        edtusuario.setfocus;
        Abort;
      end;
      varsenha := crypt('D',datamodule1.qry.fieldbyname('senha').asstring,'HELIO');
...
begin
  ...
  begin
    ...
    if recordcount = 0 then
      Application.MessageBox('Atenção! Usuário Não Cadastrado !', PChar(Application.Title),MB_OK + MB_ICONINFORMATION)
    else
    begin
      versenha := crypt('D',fieldbyname('senha').VALUE,'HELIO');
      if edtsenha.text <> versenha then
        Application.MessageBox('Atenção! Senha Inválida !', PChar(Application.Title),MB_OK + MB_ICONINFORMATION)
      else
      if fieldbyname('tipo').asinteger = 0 then
        Application.MessageBox('Atenção! Usuário Não Autorizado !', PChar(Application.Title),MB_OK + MB_ICONINFORMATION)
      else
        _retorno := 1;
    end;
  end;
...
begin
  ...
  frmautorizadesconto.Label1.Caption  := 'Autorização de Venda para Cliente Bloqueado';
  ...
  begin
    Application.MessageBox(' Atenção! Não Existe Usuário LOGADO com Permissão '+ #13#10 + ' Para Autorização! Por Favor Chamar o Fiscal de Caixa!',PChar(Application.Title), MB_OK + MB_ICONWARNING);
    Abort;
  end;

  //Criando e chamando o formulário
