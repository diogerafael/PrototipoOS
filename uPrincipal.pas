...
uses
  ...
  Vcl.ExtCtrls, cxButtons, cxControls, Vcl.ComCtrls,
  uFrmCadastrarProjeto,IniFiles,
  Vcl.AppEvnts, RDprint, JvComponentBase,
  JvThread,
  dxStatusBar, Vcl.Imaging.jpeg, cxLookAndFeels, cxLookAndFeelPainters,
...
type
  TfrmPrincipal = class(TForm)
    ...
    procedure btOSInternoClick(Sender: TObject);
  private
...
begin
  ...
  try
    ...
    dxStatusBar1.Panels[0].Text:= 'Usuário: '+DataModule1.qryusuario.FieldByName('nome').AsString;
    dxStatusBar1.Panels[1].Text:='Servidor: '+Servidor;
    dxStatusBar1.Panels[2].Text:='Release:  26/10/2015 08:14';

    ExibirAnomaliasUrbano;

  except on E: Exception do
    Application.MessageBox('O Sistema Se Comportou de Maneira Inesperada !','Atenção', MB_OK + MB_ICONWARNING);
  end;
  finally
  end;
end;
...
procedure TfrmPrincipal.btOSInternoClick(Sender: TObject);
begin
  try
    FrmOrdemServicoInterno := TFrmOrdemServicoInterno.Create(Self);
    FrmOrdemServicoInterno.ShowModal;
    FreeAndNil(FrmOrdemServicoInterno);
  except on E: Exception do
    CriarLog(Self,E,'Abrir Tela de OS!');
  end;
end;
